...
var filters = {
    ...
    "has": function(ctx, val){
        ...
        var needs = exports.select(ctx, val).map(function(e){ return hash(domUtils.getOuterHTML(e)) });

        var cmp = function(obj){
            var hashObj = hash(domUtils.getOuterHTML(obj)),
                match = false;
...
var _select = function(dom, selector) {
    ...
    var applyAttributeFilter = function(contexts, tag, attributes) {
        found = [];
        if ( tag ) {
            // Filter to only those matching the tag name
            for(var t = 0; t < contexts.length; t++){
                found = found.concat(domUtils.getElementsByTagName(tag, contexts[t], true));
            }
        ...
        if ( attributes ) {
            ...
            while(attrmatch = attrParseRe.exec(attributes)){
                ...
                for(var q = 0; q < contexts.length; q++){
                    ...
                    found = found.concat(domUtils.getElements(options, contexts[q], (tag ? false : true)));
                }
    ...
    for ( var i = 0; i < tokens.length; i++ ) {
        ...
        // Attribute and Tag selectors
            ...
            currentContext = applyAttributeFilter(currentContext, match[1], match[2]);
        }
                ...
                return {
                    'class': function (value) {
                        ...
                        for (var i = 1, len = parts.length; i < len; i++) {
                            ...
                            var part = parts[i];
                            if ((p = part.indexOf('[')) !== -1 ) {
                                var attrPart = parts[1].substring(p, parts[1].length);
                                part = part.substring(0, p);

                                if (applyAttributeFilter([context], tag, attrPart).length == 0) return false;
                            }

                            if (classes.indexOf(part) == -1) return false;
                        }
